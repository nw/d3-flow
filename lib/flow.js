...
module.exports = function(fn){
  ...
  Flow.prototype = {
    ...
    get: function(key, type){
      ...
      if(!val) return null;
      return val['selection'] || val['instance'] || val['value'];
    },
    ...
    _set: function(cmd, done){
        var key = cmd.args[0]
          , val = cmd.args[1]
          , type = cmd.args[2]
          , current = this._store[key] || {};
      if(!type) current.value = val;
      else current[type] = val;
      this._store[key] = current;
      done();
    },
    ...
    _increase: function(cmd, done){
      var val = this._get(cmd.args[0]);
      if(!val) val = {};
      if(!(typeof val.value === 'number')) val.value = 0;
      val.value++;
      console.log('increase')
      done(this._store[cmd.args[0]] = val);
    },
    
    _decrease: function(cmd, done){
      var val = this._get(cmd.args[0]);
      if(!val) val = {};
      if(!(typeof val.value === 'number')) val.value = 0;
      val.value--;
